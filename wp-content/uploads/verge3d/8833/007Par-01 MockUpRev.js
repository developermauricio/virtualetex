...
(function() {
    ...
    var sceneURL = params.load ? params.load : '__URL__007Par-01 MockUpRev.gltf'.replace('__URL__', '');
    if (!sceneURL) {
...
function runCode(app) {
    ...
    const annotationAcrilic = $("#acrilic");
    const annotationSpray = $("#spray");
    ...
    /***  Info  ***/
    const urlBase = 'https://virtualetex.creategicalatina.com';

    /***  placa  ***/
    const titlePlaca = 'PLACA Estándar';
    const urlPlacaPDF = '/wp-content/uploads/2021/08/FT-Gyplac-Placa-Estandar.pdf';
    const imgPlaca = '/wp-content/uploads/2021/08/placa.png';
    const textPlaca = `Placa de yeso compuesta por un núcleo de roca de yeso y
    aditivos, cuyas caras están revestidas por varias capas de
    papel celulosa 100% de procesos reciclados.<br><br>
    Estas son fabricadas cumpliendo las especificaciones
    descritas en la norma NTC 16159.`;
    ...
    /***  Acrílico  ***/
    const titleAcrilic = 'Sellante Cortafuego Acrílico';
    const urlAcrilicPDF = '/wp-content/uploads/2021/08/FT-PROMASEAL-A-Acrilic.pdf';
    const imgAcrilic = '/wp-content/uploads/2021/10/acrilic.jpg';
    const textAcrilic = `PROMASEAL®-A es un sellante cortafuego acrílico
    monocomponente. Juntas estáticas o sin movimiento se
    pueden sellar de forma rápida gracias a sus cualidades de
    aplicación. Una pintura común de emulsión o acrílica puede
    ser utilizada para colorear o pintar las juntas.`;

    /***  Spray  ***/
    const titleSpray = 'Recubrimiento Cortafuego';
    const urlSprayPDF = '/wp-content/uploads/2021/08/FT-PROMASEAL-A-SPRAY.pdf';
    const imgSpray = '/wp-content/uploads/2021/10/spray.jpg';
    const textSpray = `PROMASEAL®-A Spray es un sellante acrílico,
    monocomponente diseñado para impedir la propagación
    de humo y llamas en sistemas de sellado de juntas y pasos
    de instalaciones. Forma un sello flexible una vez está seco,
    es adecuado para acondicionar movimiento y altamente
    ...
    annotationAcrilic.on("click", () => { openMuroPopup(titleAcrilic, urlAcrilicPDF, imgAcrilic, textAcrilic) });	
    annotationSpray.on("click", () => { openMuroPopup(titleSpray, urlSprayPDF, imgSpray, textSpray) });	

    closePopup.addEventListener("click", closeMuroPopup, false);
    //$('#mainContent').on("click", closeMuroPopup);
}
