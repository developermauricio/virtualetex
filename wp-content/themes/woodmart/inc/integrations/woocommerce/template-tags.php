...
if( ! function_exists( 'woodmart_my_account_wrapp_start' ) ) {
	function woodmart_my_account_wrapp_start(){ //TODO: aqui inicia la pagina de mi cuenta
		//TODO: Registrar o login user, rediriguir al usuaria al apartamento
		...
		if ( $afreg_extra_fields ) {
			foreach ($afreg_extra_fields as $afreg_field) {
				...
				if ( $afreg_field_type == 'text' ) {
					$fullName = get_user_meta( intval($user->ID), 'afreg_additional_' . intval($afreg_field->ID), true );	
				}
				if ( $afreg_field_type == 'select' ) {
					$profesion = get_user_meta( intval($user->ID), 'afreg_additional_' . intval($afreg_field->ID), true );
				}			
			}
		}
		...
		?>
			<script>
				const newUser = <?php echo json_encode($userData); ?>;

				jQuery.ajax({
					url: 'https://backend-etex.creategicalatina.com/api/register-or-login',
					type: "POST",
					data: newUser,
					success: function (response) {
						console.log('responce: ', response);   
						location.href = 'https://virtualetex.creategicalatina.com/apartamento-escandinavo/';         
					},
...
if( ! function_exists( 'woodmart_get_header_links' ) ) {
	//TODO: aqui salen las opciones del menu para inicar sesión
