...
<div class="wd-header-my-account wd-tools-element wd-event-hover <?php echo esc_attr( $classes ); ?>">
	...
	<?php $currentUser = wp_get_current_user(); // TODO: get email user
		if ($currentUser) { ?>
			<input id="currentUserEmail" name="current_user_email" type="hidden" value="<?php echo $currentUser->user_email ?>" />
			<?php	
		} else { ?>
			<input id="currentUserEmail" name="current_user_email" type="hidden" value="" />
	<?php } ?>
</div>
